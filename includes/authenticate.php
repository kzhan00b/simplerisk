...
function set_user_permissions($user, $pass, $upgrade = false)
{
    ...
    if (!$upgrade)
    {
        ...
        if (get_setting('strict_user_validation') == 0)
        {
            ...
            $stmt = $db->prepare("SELECT value, type, name, lang, assessments, asset, admin, general, custom_display_settings, review_veryhigh, review_high, review_medium, review_low, review_insignificant, submit_risks, modify_risks, plan_mitigations, close_risks FROM user WHERE LOWER(convert(`username` using utf8)) = LOWER(:user)");
        }
        else
        {
            // Query the DB for the users complete information
            $stmt = $db->prepare("SELECT value, type, name, lang, assessments, asset, admin, general, custom_display_settings, review_veryhigh, review_high, review_medium, review_low, review_insignificant, submit_risks, modify_risks, plan_mitigations, close_risks FROM user WHERE username = :user");
        }
    }
    ...
    if (!$upgrade)
    {
        ...
        $_SESSION['general'] = $array[0]['general'];
